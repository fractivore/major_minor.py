...
mono_audio_data = np.array([data_point[0] for data_point in audio])
...
starting_freq_spectrum = np.fft.fft(mono_audio_data)
...
def convert_key(conversion_dict):
    ...
    while current_duration + BUFFER_SIZE < mono_audio_data.size:
        next_duration = current_duration + BUFFER_SIZE
        print("current_duration:", current_duration, "next_duration:", next_duration)
        currentAudioBin = mono_audio_data[current_duration:next_duration] # audio is stereo; just grab one channel
        current_duration = next_duration

        # Compute the FFT to get the frequency-domain representation
        frequency_spectrum = np.fft.fft(currentAudioBin)
        print("frequency_spectrum.size:", frequency_spectrum.size)
        ...
        for f in range(int(BUFFER_SIZE / 2)):
            ...
            if current_pitch in target_notes:
                ...
                temp = frequency_spectrum[new_key_f]
                frequency_spectrum[new_key_f] = frequency_spectrum[f]
                frequency_spectrum[symmetric_new_key_f] = frequency_spectrum[symmetric_f]
                frequency_spectrum[f] = temp
                frequency_spectrum[symmetric_f] = temp

        # Take the inverse FFT to get the modified audio back, and append it to running wav data list
        modified_wav_clip = np.real(np.fft.ifft(frequency_spectrum))
        modified_wav_clip = np.float32(modified_wav_clip).tolist()
        wav_data_out = wav_data_out + modified_wav_clip
    return wav_data_out
